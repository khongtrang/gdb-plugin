...
class PEDA(object):
    ...
    def read_int(self, address, intsize=None):
        ...
        if value:
            value = int(codecs.encode(value[::-1], 'hex'),16)
            return value
        else:
            return None
    ...
    def parse_struct(self,filename):
        ...
        p_var = re.compile(r'\{(.+?)\}', flags=re.DOTALL)
        p_name = re.compile(r'struct.(.+?)\n{', flags=re.DOTALL)
        data  = p_var.findall(input)
        names = p_name.findall(input)
        structs = {}
        no=len(names)
        for i in range(no):
            elements = data[i].strip().strip(";").split(";\n  ")
            name_bits = []
            for type in elements:
                types = type.split(" ")
                if len(types) == 3 : #struct NODE *list[10]
                    n = types[2].find("[")
                    m = types[2].find("]")
                    k = 1
                    if (n!=m) & (n!=-1) & (m!=-1) :
                        k = int(types[2][n+1:m])
                    name_bits.append([types[2],int(arch/8),k])
                    continue

                if len(types) == 2: #_QWORD': p_node[12]
                    n = types[1].find("[")
                    m = types[1].find("]")
                    k = 1
                    if (n!=m) & (n!=-1) & (m!=-1) :
                        k = int(types[1][n+1:m])
                    name_bits.append([types[1],get_byte(types[0],arch),k])
    ...
    def malloc_chunk(self,addr,isTop=0,debug=0):
        ...
        if isTop:
            show += green(" TOP_CHUNK")
            print(show)
            return chunk
        headersize = 8
        if self.getarch()[1] == 64:
            headersize = 16
    ...
    def bins(self,addr=None):
        ...
        num_bins = int((bins.type.sizeof / bins.type.target().sizeof)/ 2)
        print(yellow('UNSORTBINS : '))
        for k in range(num_bins):
            i = k*2
            if "main_arena" in str(bins[i]):
                continue
            addr = int(bins[i])
            if self.is_address(addr):
                p = addr
                print("bins %d : "%i)
                while True:
                    p = self.malloc_chunk(p)["fd"]
                    if p==None:
                        return
                    p = int(p)
                    if not self.is_address(p):
                        error_msg("0x%x is not a address"%p)
                    # check p->fd->data == addr
                    is_bin_addr = p + size_t_size*2     
                    if self.read_int(p+size_t_size*2,size_t_size) == addr: #fixed read_int
                        break
            else:
                error_msg("0x%x is not a address"%p)
            ...
            while addr:
                ...
                if self.is_address(addr):
                    ...
                    if addr!=0:
                        ...
                        break
    ########### heap 4 fun ##############
    def check_unlink(self,chunk_addr):
        print(green("Check unlink 0x%x"%chunk_addr))
        unlink_chunk = self.format_chunk(chunk_addr)
        fd_addr = int(unlink_chunk["fd"])
        fd_chunk = self.format_chunk(fd_addr)
        bk_addr = int(unlink_chunk["bk"])
        bk_chunk = self.format_chunk(bk_addr)
        fd_bk_addr = int(fd_addr["bk"])
        bk_fd_addr - int(bk_chunk["fd"])
        if fd_bk_addr != chunk_addr:
            error_msg("failed check unlink")
            print(green("chunk->FD->BK != chunk [ 0x%x-> 0x%x -> 0x%x != 0x%x ]"%(chunk_addr,fd_addr,fd_bk_addr,chunk_addr)))
            return
        if(bk_fd_addr != chunk_addr):
            error_msg("failed check unlink")
            print(green("chunk->BK->FD != chunk [ 0x%x-> 0x%x -> 0x%x != 0x%x ]"%(chunk_addr,bk_addr,bk_fd_addr,chunk_addr)))
            return


    def check_free(self,chunk_addr):
        size_t_size = gdb.lookup_type('size_t').sizeof
        chunk = self.format_chunk(chunk_addr)
        topaddr = int(self.get_main_arena()['top'])
        topchunk = self.format_chunk(topaddr)
        size = int(chunk["size"]) & ~7
        prev_size = int(chunk["prev_size"])
        status = int(chunk["size"]) & 7
        topsize = int(topchunk["size"])
        if status & self.IS_MMAPED == 2:
            warning_msg("this chunk is mmaped")
        ...
        if size <= 10*size_t_size :
            ...
            print(blue("you are freeing UNSORT chunk"))
        if chunk_addr == topaddr:
            error_msg("this chunk is top chunk")
            return
        if (chunk_addr + size) >= (topaddr + topsize):
            error_msg("chunk+size > top+sizetop,fix at size_addr: 0x%x"%(chunk_addr+size_t_size))
            return
        if self.chunk_inuse(chunk_addr) != True:
            error_msg("chunk not inused")
            print(green("fix it at (chunk_addr+size)->status [0x%x + 0x%x +0x%x = 0x%x]")%(chunk_addr,size,size_t_size,chunk_addr+size+size_t_size) )
            return
        nextchunk_addr = chunk_addr+size
        nextchunk = self.format_chunk(nextchunk_addr)
        nextsize = int(nextchunk["size"])& ~7
        for vmap in self.get_vmmap():
            if vmap[3] == "[stack]":
                sys_mem = vmap[1]
                if nextsize > sys_mem:
                    error_msg("nextchunk_size > system_mem")
                    print(green("fix it at next_size:(chunk_addr+size)->size [0x%x + 0x%x +0x%x = 0x%x]")%(chunk_addr,size,size_t_size,chunk_addr+size+size_t_size) )
                    return
        if nextsize < 8:
            error_msg("invalid nextchunk_size")
            print(green("fix it at next_size:(chunk_addr+size)->size [0x%x + 0x%x +0x%x = 0x%x]")%(chunk_addr,size,size_t_size,chunk_addr+size+size_t_size) )
            return
        if status & self.PREV_INUSE != 1:
            addr_bk = chunk_addr - prev_size
            print(blue("Try consolidate backward_chunk,unlink_chunk = 0x%x"%addr_bk))
            self.check_unlink(addr_bk)
        else:
            print(blue("backward_chunk inused,you can change flag inused at: 0x%x"%(chunk_addr+size_t_size)))
        if nextchunk_addr == topaddr:
            warning_msg("nextchunk is topchunk: 0x%x"%topaddr)
            return
        if self.chunk_inuse(nextchunk_addr) != True:
            print(blue("Trying consolidate forward_chunk,unlink_chunk = 0x%x "%nextchunk_addr))
            self.check_unlink(addr_bk)
        else:
...
class PEDACmd(object):
    ...
    def heap_checkfree(self,*arg):
        """
        Print info before free a chunk
        Usage:
        heap checkfree addr
        """
        if len(arg) == 1:
            addr = int(arg[0],16)
            peda.check_free(addr)
        return
    ...
    def reload(self, *arg):
        """
        Reload PEDA sources, keep current_instructionnt options untouch
        Usage:
            MYNAME [name]
        """
    ...
    def context(self, *arg):
        ...
        if "SIG" in status:
            print("Stopped reason: %s" % red(status))
        return
    ...
    def skeleton(self, *arg):
		...
		type = xpl
        """
        options = ["argv", "stdin", "env", "remote","xpl"]
        (opt, outfile,host,port) = normalize_argv(arg, 4)
        ...
        if opt=="xpl":        
            code=ExploitSkeleton().xpl
            code=code.replace("HOST=","HOST = '{0}'".format(host))
            code=code.replace("PORT=","PORT = {0}".format(port))
            filename = './'+peda.getfile().split('/')[-1]
            code=code.replace("{2}","'{0}'".format(filename))
        if outfile:
